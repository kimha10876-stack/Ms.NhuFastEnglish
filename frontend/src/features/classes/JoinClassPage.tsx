...
export default function JoinClassPage() {
  ...
  return (
    <div className="min-h-svh bg-muted flex flex-col items-center justify-center px-4 py-10">

      {/* Logo */}
      <div className="flex items-center gap-2.5 mb-8">
        <div className="w-9 h-9 rounded-full overflow-hidden flex items-center justify-center shrink-0 border border-border bg-background">
          <img src="/logo.png" alt="Ms Nhu Fast English Logo" className="w-full h-full object-cover" />
        </div>
        <span className="font-bold text-[17px] tracking-tight text-ink-900">Ms Nhu Fast English</span>
      </div>

      <div className="bg-background rounded border border-border shadow-sm w-full max-w-sm">
        ...
        {isError && (
          <div className="flex flex-col items-center py-12 px-7 text-center">
            <div className="w-14 h-14 rounded bg-red-50 flex items-center justify-center mb-4">
              <AlertCircle className="h-7 w-7 text-red-500" />
            </div>
            <h2 className="font-bold text-[18px] text-ink-900 mb-1">Link không hợp lệ</h2>
        ...
        {isSuccess && (
          <div className="flex flex-col items-center py-12 px-7 text-center">
            <div className="w-14 h-14 rounded bg-emerald-50 flex items-center justify-center mb-4">
              <CheckCircle className="h-7 w-7 text-emerald-500" />
            </div>
            <h2 className="font-bold text-[18px] text-ink-900 mb-1">Tham gia thành công!</h2>
            <p className="text-sm text-muted-foreground mb-6">
              Bạn đã vào lớp <span className="font-semibold text-ink-900">{info?.className}</span>.
            </p>
          ...
          <div className="p-7">
            ...
            <div className="rounded border border-border overflow-hidden mb-5">
              ...
              <div className="px-4 py-4">
                <h2 className="font-bold text-[18px] text-ink-900 mb-0.5">{info.className}</h2>
                <p className="text-sm text-muted-foreground mb-3">{info.teacherName}</p>
            ...
            {/* Logged in */}
            {isLoggedIn && (
              <Button className="w-full h-11 text-[15px]" onClick={() => join(token, { onSuccess: () => setTimeout(() => navigate('/classes'), 1500) })} loading={joining}>
                Tham gia lớp học
              </Button>
            )}
