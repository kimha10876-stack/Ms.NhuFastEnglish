  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <button
        ...
        disabled={disabled}
        className={`w-full flex items-center justify-between h-9 px-3 rounded border bg-background text-sm text-ink-900 transition-all outline-none text-left
          ${disabled ? 'opacity-50 cursor-not-allowed bg-muted' : 'cursor-pointer hover:border-primary-400'}
          ${isOpen ? 'border-primary-500 ring-2 ring-primary-500/20' : error ? 'border-red-500' : 'border-border'}
        `}
      >
        {selectedTeacher ? (
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-xs text-muted-foreground truncate hidden sm:inline">({selectedTeacher.email})</span>
          </div>
        ) : (
          <span className="text-muted-foreground">Chọn giáo viên phụ trách...</span>
        )}
      ...
      {/* ── Dropdown Menu ── */}
      {isOpen && (
        <div className="absolute left-0 right-0 mt-1.5 z-[100] bg-background border border-border rounded shadow-xl overflow-hidden max-h-[300px] flex flex-col animate-in fade-in slide-in-from-top-1 duration-150">
          {/* Search Box */}
          <div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-muted shrink-0">
            ...
            <input
              ...
              className="w-full bg-transparent text-sm text-ink-900 border-none outline-none placeholder-gray-400 py-0.5"
            />
          </div>
              ...
              filteredTeachers.map((t) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 hover:bg-primary-50/50 transition-colors text-left
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-semibold truncate ${isSelected ? 'text-primary-700' : 'text-ink-900'}`}>
                        {t.fullName}
                      </p>
                      <p className="text-xs text-muted-foreground truncate">{t.email}</p>
                    </div>
                    {isSelected && (
                      <Check className="h-4 w-4 text-primary-600 shrink-0 ml-auto" />
                    )}
                  </button>
                )
              })
