...
<EmptyState
          icon={AlertTriangle}
          title="Không tìm thấy thông tin bài tập"
          description="Bài tập không tồn tại hoặc bạn không có quyền truy cập."
          action={
            <Button onClick={() => navigate(-1)} className="rounded font-bold">
              Quay lại
            </Button>
          }
        />
  ...
  const renderInlineSelectQuestion = (q: AssignmentQuestion, studentAns: string) => {
      ...
      return (
        <div className="space-y-2">
          ...
          <Input
            ...
            className="rounded h-11 text-xs bg-background border border-border"
          />
        </div>
      )
    }
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      parts.push(
        <select
          ...
          disabled={hasSubmitted || isBlocked}
          onChange={(e) => setQuizAnswers({ ...quizAnswers, [q.id]: e.target.value })}
          className={`mx-1 inline-block px-2.5 py-1 text-xs font-bold rounded border outline-none bg-background transition-all cursor-pointer ${
            hasSubmitted
              ? (studentAns === q.correctAnswer
                  ? 'border-emerald-500 text-emerald-800 bg-emerald-50/50'
                  : 'border-red-500 text-red-800 bg-red-50/50')
              : 'border-border focus:border-primary-500 focus:ring-1 focus:ring-primary-500/20'
          }`}
        >
    ...
    return (
      <div className="text-sm font-semibold text-foreground text-left leading-relaxed flex flex-wrap items-center gap-y-1.5 p-3.5 bg-muted/40 rounded-[8px] border border-border">
        {parts}
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col font-sans select-none">
      ...
      <header className="bg-slate-900 text-white py-3.5 px-6 flex items-center justify-between border-b border-slate-800 shrink-0 sticky top-0 z-40 shadow-md">
        <div className="flex items-center gap-3">
          <button 
            type="button"
            onClick={() => navigate(-1)} 
            className="p-2 rounded bg-slate-800/80 hover:bg-slate-800 text-slate-300 hover:text-white transition-all"
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="bg-primary-500 text-slate-900 font-extrabold text-xs uppercase px-2 py-0.5 rounded tracking-wider">
                {assignment.assignmentType}
              </span>
              {isPreview && (
                <span className="bg-blue-600 text-white font-extrabold text-xs uppercase px-2 py-0.5 rounded tracking-wider flex items-center gap-1">
        ...
        {/* Live Timer Widget */}
        <div className="flex items-center gap-2.5 bg-slate-800/60 border border-slate-700/80 px-4 py-1.5 rounded">
          <Clock className={`h-4.5 w-4.5 \${isOverdue && !hasSubmitted ? 'text-red-500 animate-pulse' : 'text-primary-500'}`} />
          <div className="text-right">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Thời gian còn lại</p>
            <p className={`text-xs font-black tracking-wide \${isOverdue && !hasSubmitted ? 'text-red-500' : 'text-primary-400'}`}>
              {timeLeft}
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <aside className="w-full md:w-80 bg-background border-b md:border-b-0 md:border-r border-border p-5 flex flex-col justify-between shrink-0 overflow-y-auto">
          <div className="space-y-6">
            
            {/* Class Assignment Info */}
            <div className="space-y-2">
              <h2 className="text-xs font-black text-muted-foreground uppercase tracking-wider">Mô tả bài tập</h2>
              <div className="bg-muted border border-border p-3.5 rounded text-xs text-foreground font-semibold leading-relaxed whitespace-pre-wrap">
            ...
            {assignment.assignmentType === 'Quiz' && questions.length > 0 && (
              <div className="space-y-3.5">
                <div className="flex items-center justify-between">
                  <h2 className="text-xs font-black text-muted-foreground uppercase tracking-wider">Tiến độ làm bài</h2>
                ...
                <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-primary-400 to-primary-500 transition-all duration-300 rounded-full" 
                ...
                <div className="grid grid-cols-5 gap-2 pt-2">
                  {questions.map((q, idx) => {
                    ...
                    let bgClass = 'border-border text-muted-foreground hover:border-gray-400'
                    if (isAnswered) bgClass = 'bg-primary-50 border-primary-400 text-primary-700 font-black'
                    ...
                    return (
                      <button
                        ...
                        className={`h-10 rounded border text-xs font-bold transition-all flex items-center justify-center ${bgClass}`}
                      >
                        {idx + 1}
                      </button>
                    )
            ...
            {isOverdue && assignment.allowLateSubmission && !hasSubmitted && (
              <div className="bg-primary-50 border-l-4 border-primary-500 p-3 rounded">
                <p className="text-xs text-primary-700 font-bold flex items-center gap-1.5 leading-relaxed">
            ...
            {isGraded && assignment.submission && (
              <div className="bg-emerald-50 border border-emerald-200 p-4 rounded flex gap-3.5 items-start">
                ...
                <div>
                  <h4 className="text-xs font-bold text-emerald-800 uppercase tracking-wider">Kết quả chấm</h4>
          ...
          {(!hasSubmitted || assignment.submission?.grade === null) && !isBlocked && (
            <div className="pt-6 border-t border-border mt-6 shrink-0">
              <Button 
                ...
                className="w-full gap-1.5 font-bold py-3 shadow-md shadow-primary-500/20 bg-gradient-to-r from-primary-500 to-primary-600 text-white hover:from-primary-600 hover:to-primary-700"
              >
        ...
        <main className="flex-1 bg-muted p-6 overflow-y-auto scroll-smooth">
                ...
                {questions.map((q, idx) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`p-5 bg-background border rounded space-y-4 shadow-sm transition-all duration-300 ${
                    ...
                    >
                      ...
                      {q.type !== 'FillInTheBlank' && (
                        <h3 className="font-extrabold text-sm text-foreground leading-relaxed text-left">
                          {q.questionText}
                        </h3>
                      )}
                      ...
                      {q.type === 'MultipleChoice' && q.options && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                          {['A', 'B', 'C', 'D'].map((choice, oIdx) => {
                            ...
                            let choiceStyle = 'border-border bg-background text-foreground hover:bg-muted/50'
                            if (isSelected) choiceStyle = 'border-primary-500 bg-primary-50/70 text-primary-900 shadow-sm'
 ...
 <label
                                key={choice}
                                htmlFor={optionId}
                                className={`flex items-start gap-2.5 p-3 rounded border text-xs font-semibold cursor-pointer transition-all ${choiceStyle}`}
                              >
                                <input
                                  type="radio"
                                  id={optionId}
 ...
 <label
                                key={choice}
                                htmlFor={optionId}
                                className={`flex items-center gap-2 px-5 py-2.5 rounded border text-xs font-bold cursor-pointer transition-all ${choiceStyle}`}
                              >
                                <input
                                  type="radio"
                                  id={optionId}
                      ...
                      {q.type === 'FillInTheBlank' && (
                        <div className="space-y-1.5">
                          ...
                          {hasSubmitted && (
                            <p className="text-xs font-bold text-muted-foreground text-left mt-1.5">
                      ...
                      {q.type === 'Writing' && (
                        <textarea
                          ...
                          className="w-full min-h-[140px] p-4 text-xs font-medium rounded-[8px] border border-border focus:border-primary-500 focus:ring-primary-500/20 bg-background"
                        />
                      )}

                      {/* Instructor feedback for specific question */}
                      {savedAnsObj?.teacherFeedback && (
                        <div className="bg-primary-50/40 border border-primary-200/20 p-3 rounded text-xs text-muted-foreground font-semibold italic text-left flex items-start gap-2">
                          <span className="font-bold text-primary-600 shrink-0">Nhận xét của GV:</span>
              ...
              <div className="bg-background border border-border rounded-3xl p-6 shadow-sm space-y-6">
                ...
                {!hasSubmitted && !isBlocked && (
                  <div className="flex border border-border rounded overflow-hidden p-1 bg-muted/50">
                    <button
                      ...
                      className={`flex-1 py-2 text-xs font-extrabold rounded transition-all ${
                        submissionMode === 'link'
                ...
                {(hasSubmitted ? !!submitForm.fileUrl : submissionMode === 'link') ? (
                  <div className="space-y-4">
                    <div className="space-y-2">
 <label className="text-xs uppercase tracking-wider block text-left">Ghi chú hoặc nội dung bài làm</label>
                      <textarea
                        ...
                        className="w-full min-h-[140px] p-4 text-sm font-medium bg-background rounded-[8px] border border-border focus:border-primary-500 focus:ring-primary-500/20 text-left"
                        disabled={hasSubmitted || isBlocked}
                      />
                    </div>

                    <div className="space-y-2 text-left">
 <label className="text-xs uppercase tracking-wider block">Đường dẫn bài làm (Link Google Drive, Canva, Figma...)</label>
                      <Input
                        ...
                        className="rounded h-11 text-xs bg-background border border-border focus:border-primary-500"
                        required
                        disabled={hasSubmitted || isBlocked}
                      />
                    </div>
                  ...
                  <div className="space-y-2">
 <label className="text-xs uppercase tracking-wider block text-left">Nội dung bài làm tự luận</label>
                    <textarea
                      value={submitForm.submissionText}
                      onChange={(e) => setSubmitForm({ ...submitForm, submissionText: e.target.value })}
                      placeholder="Viết bài tự luận hoặc câu trả lời của bạn trực tiếp tại đây..."
                      className="w-full min-h-[260px] p-4 text-sm font-medium bg-background rounded-[8px] border border-border focus:border-primary-500 focus:ring-primary-500/20 text-left"
                      required
                      disabled={hasSubmitted || isBlocked}
                    />
                  </div>
                ...
                {submitForm.fileUrl && hasSubmitted && (
                  <div className="pt-2 text-left">
                    <a 
                      ...
                      className="inline-flex items-center gap-1.5 text-xs text-primary-600 hover:text-primary-700 font-bold bg-primary-50/50 border border-primary-100 px-3.5 py-1.5 rounded transition-all"
                    >
